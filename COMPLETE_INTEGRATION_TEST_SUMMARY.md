...
각 케이스는 **다른 비즈니스 규칙**을 검증하며, **독립적인 테스트 메서드**로 구현:

| 테스트 케이스 | 검증 대상 | 예외 타입 | HTTP 상태 |
|-------------|---------|---------|----------|
| **잔액 부족** | Balance 도메인 검증 | `DomainConflictException` | 409 Conflict |
| **권한 없음** | 예약자 본인 확인 로직 | `DomainForbiddenException` | 403 Forbidden |
| **중복 결제** | 멱등성(Idempotency) | `DomainConflictException` | 409 Conflict |
| **취소된 예약** | 예약 상태 검증 | `DomainConflictException` | 409 Conflict |

**상세 시나리오**:
- ✅ **결제 실패 - 잔액 부족**: 30,000원만 있을 때 50,000원 좌석 결제 시도
- ✅ **결제 실패 - 권한 없음**: User B가 User A의 예약을 결제 시도 (본인만 결제 가능)
- ✅ **결제 실패 - 중복 결제**: 이미 결제 완료된 예약을 다시 결제 시도
- ✅ **결제 실패 - 취소된 예약**: 취소 상태의 예약을 결제 시도
...
| 테스트 케이스 | 검증 대상 | 예외 타입 | HTTP 상태 |
|-------------|---------|---------|----------|
| **권한 없음** | 결제자 본인 확인 로직 | `DomainForbiddenException` | 403 Forbidden |
| **중복 환불** | 멱등성(Idempotency) | `DomainConflictException` | 409 Conflict |

**상세 시나리오**:
- ✅ **환불 실패 - 권한 없음**: User B가 User A의 결제를 환불 시도 (본인만 환불 가능)
- ✅ **환불 실패 - 중복 환불**: 이미 환불 완료된 결제를 다시 환불 시도
