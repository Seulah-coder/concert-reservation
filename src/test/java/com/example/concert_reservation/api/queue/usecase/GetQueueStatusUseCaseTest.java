...
class GetQueueStatusUseCaseTest {
    ...
    void execute_waitingQueue_includesWaitingAhead() {
        ...
        when(queueValidator.countWaitingAheadByToken(waitingQueue.getToken().getValue())).thenReturn(3L);
        ...
        verify(queueValidator).countWaitingAheadByToken(waitingQueue.getToken().getValue());
    }
    ...
    void execute_activeQueue_waitingAheadIsZero() {
        ...
        verify(queueValidator).validateAndGetQueue(any(QueueToken.class));
        verify(queueValidator, never()).countWaitingAheadByToken(anyString());
    }
    ...
    void execute_firstInLine_waitingAheadIsZero() {
        ...
        when(queueValidator.countWaitingAheadByToken(firstQueue.getToken().getValue())).thenReturn(0L);
